...
import com.frangerapp.network.multipart.httphelpers.util.ByteArrayBuffer;
...
public class FDHttpMultipart {
    ...
    private final List<FDFormBodyPart> parts;

    private final FDHttpMultipartMode mode;
     ...
     */
    public FDHttpMultipart(final String subType, final Charset charset, final String boundary, FDHttpMultipartMode mode) {
        super();
        if (subType == null) {
        ...
        this.parts = new ArrayList<FDFormBodyPart>();
        this.mode = mode;
    }

    /**
     * Creates an instance with the specified settings.
     * Mode is set to {@link FDHttpMultipartMode#STRICT}
     *
     * @param subType mime subtype - must not be {@code null}
     * @param charset the character set to use. May be {@code null}, in which case {@link MIME#DEFAULT_CHARSET} - i.e. US-ASCII - is used.
     * @param boundary to use  - must not be {@code null}
     * @throws IllegalArgumentException if charset is null or boundary is null
     */
    public FDHttpMultipart(final String subType, final Charset charset, final String boundary) {
        this(subType, charset, boundary, FDHttpMultipartMode.STRICT);
    }

    public FDHttpMultipart(final String subType, final String boundary) {
    ...
    public FDHttpMultipartMode getMode() {
        return this.mode;
    }

    public List<FDFormBodyPart> getBodyParts() {
        return this.parts;
    }

    public void addBodyPart(final FDFormBodyPart part) {
        if (part == null) {
            return;
        }
        this.parts.add(part);
